...
def PlotFitnessSample(df_fitness_data_1, df_fitness_data_2, df_fitness_data_3, exp_folder, max_generations):
    #destination_file=sample_folder+"/BE_plot"+PLOT_EXTENSION
    destination_file=exp_folder+"/StackedFitness"
    ...
    for generation in range(max_generations):
        ...
        generation_fitness=[]
        for walk_id in df_fitness_data_1:
            fitness_val=-1 #init
            if(generation<len(df_fitness_data_1[walk_id])):
                fitness_val=df_fitness_data_1[walk_id][generation]
                last_fitness_1[walk_id]=fitness_val
            else:
                ...
                fitness_val=last_fitness_1[walk_id]
            #print(fitness_val)
            generation_fitness.append(fitness_val)

        best_fitness_1.append(max(generation_fitness))
        mean_fitness_1.append(np.mean(generation_fitness))
        mean_fitness_var_1.append(np.var(generation_fitness))

    x_count=range(1,max_generations+1)
    #ax_be = plt.gca()

    #ax_be.plot(x_count, mean_fitness_1,"--",label="L=30", color="red")
    #Use what would be the default axis limits if we used only the y value
    #ylim=ax_be.get_ylim()
    #ax_be.clear()

    #ax_be.plot(mean_fitness,label="mean")
    plt.errorbar(x_count, mean_fitness_1, yerr=mean_fitness_var_1, ecolor='silver', color="red")
    #ax_be.set_ylim(ylim)
    ...
    for generation in range(max_generations):
        #print("generation "+str(generation))
        generation_fitness=[]
        for walk_id in df_fitness_data_2:
            fitness_val=-1 #init
            if(generation<len(df_fitness_data_2[walk_id])):
                fitness_val=df_fitness_data_2[walk_id][generation]
                last_fitness_2[walk_id]=fitness_val
            else:
                #terminating early means solution was found (ideal fitness)
                #we keep it to ensure it gets counted in the mean fitness across walks
                fitness_val=last_fitness_2[walk_id]
            #print(fitness_val)
            generation_fitness.append(fitness_val)

        best_fitness_2.append(max(generation_fitness))
        mean_fitness_2.append(np.mean(generation_fitness))
        mean_fitness_var_2.append(np.var(generation_fitness))
    ...
    #ax_be.plot(mean_fitness,label="mean")
    plt.errorbar(x_count, mean_fitness_2, yerr=mean_fitness_var_2, ecolor='silver', color="green")
    #ax_be.set_ylim(ylim)
    ...
    for generation in range(max_generations):
        #print("generation "+str(generation))
        generation_fitness=[]
        for walk_id in df_fitness_data_3:
            fitness_val=-1 #init
            if(generation<len(df_fitness_data_3[walk_id])):
                fitness_val=df_fitness_data_3[walk_id][generation]
                last_fitness_3[walk_id]=fitness_val
            else:
                #terminating early means solution was found (ideal fitness)
                #we keep it to ensure it gets counted in the mean fitness across walks
                fitness_val=last_fitness_3[walk_id]
            #print(fitness_val)
            generation_fitness.append(fitness_val)

        best_fitness_3.append(max(generation_fitness))
        mean_fitness_3.append(np.mean(generation_fitness))
        mean_fitness_var_3.append(np.var(generation_fitness))
    ...
    #ax_be.plot(mean_fitness,label="mean")
    plt.errorbar(x_count, mean_fitness_3, yerr=mean_fitness_var_3, ecolor='silver', color="blue")
    #ax_be.set_ylim(ylim)
...
def main(args):
    ...
    sample_fitness_1=dict()
    sample_fitness_2=dict()
    sample_fitness_3=dict()
    walk_fitness_1=dict()
    walk_fitness_2=dict()
    walk_fitness_3=dict()
    #experiment_type=ExperimentType.UNKNOWN
    ...
    experiment_1_folder=BuildExperimentFolderStr(root_folder,exp_id_1)
    experiment_2_folder=BuildExperimentFolderStr(root_folder,exp_id_2)
    experiment_3_folder=BuildExperimentFolderStr(root_folder,exp_id_3)



    #experiment_folder=BuildExperimentFolderStr(root_folder,experiment_id)

    experiment_details=ValidateExperimentType(mysql_config,exp_id_1)
    ...
    df_samples_1=RetrieveSamples(mysql_config,exp_id_1)
    df_samples_2=RetrieveSamples(mysql_config,exp_id_2)
    df_samples_3=RetrieveSamples(mysql_config,exp_id_3)

    #Retrieve all sample data
    for sample_id in df_samples_1["ID"]:
        #sample_id=sample_id_arr[0]
        sample_folder=BuildSampleFolderStr(experiment_1_folder, sample_id)
        ...
        for walk_id in df_walks["ID"]:
            ...
            if experiment_type == ExperimentType.BE:
                walk_fitness_1[walk_id]=PlotFitness(walk_folder)
        ...
        #        PlotFitnessSample(walk_fitness, sample_folder, max_generations)

    #Retrieve all sample data
    for sample_id in df_samples_2["ID"]:
        #sample_id=sample_id_arr[0]
        sample_folder=BuildSampleFolderStr(experiment_2_folder, sample_id)
        if experiment_type == ExperimentType.BE:
            sample_measures[sample_id]=RetrieveBESampleData(mysql_config,sample_id)
        ...
        for walk_id in df_walks["ID"]:
            ...
            #Build the plots for each walk
            if experiment_type == ExperimentType.BE:
                walk_fitness_2[walk_id]=PlotFitness(walk_folder)
    ...
    #Retrieve all sample data
    for sample_id in df_samples_3["ID"]:
        #sample_id=sample_id_arr[0]
        sample_folder=BuildSampleFolderStr(experiment_3_folder, sample_id)
        if experiment_type == ExperimentType.BE:
            sample_measures[sample_id]=RetrieveBESampleData(mysql_config,sample_id)
        ...
        for walk_id in df_walks["ID"]:
            #walk_id=walk_id_arr[0]
            walk_folder3=sample_folder+"/walk_"+str(walk_id)

            #Build the plots for each walk
            if experiment_type == ExperimentType.BE:
                walk_fitness_3[walk_id]=PlotFitness(walk_folder)
    ...
    PlotFitnessSample(walk_fitness_1, walk_fitness_2, walk_fitness_3, experiment_1_folder, max_generations)
