...
def ValidateExperimentType(mysql_config,experiment_id):
    try:
        statement = "SELECT e.ID, e.type, e.track_variance, p.name, p.bnk_gates, p.bnk_inputs, a.generations, e.bnk_source_exp_id, a.population_size FROM experiment as e inner join problem as p on p.ID=e.fk_problem_id inner join algorithm as a on a.ID=e.fk_algorithm_id where e.ID = %(exp_id)s"
...
def PlotFitnessES(walk_folder,  exp_pop_size, max_generations):
    pop_file = walk_folder+"/vre.csv"

    #load data
    pop_data=pd.read_csv(pop_file)

    ######## OVERWRITE FOR LOCAL TESTS
    #max_generations=10
    #########################################

    title_bnk="Fitness evolution of a single walk"
    destination_file=walk_folder+"/FitnessES"
    ...
    #First, compute average fitness per generation
    for it_gen in range(max_generations):
        #print("generation:"+str(it_gen))
        gen_fit=list()
        for it_ind in range(exp_pop_size):
            #print("ind:"+str(it_ind))
            ind_loc=it_ind+(it_gen*exp_pop_size)
            #print("ind loc:"+str(ind_loc))

            #print("ind data:")
            #print(pop_data.iloc[ind_loc]["Fitness"])
            ind_fit=pop_data.iloc[ind_loc]["Fitness"]
            gen_fit.append(ind_fit)
        #print(gen_fit)
        #print(np.mean(gen_fit))
        fit_data.append(np.mean(gen_fit))
        fit_std.append(np.std(gen_fit))
    ...
    #ax_bnk.plot(fit_data)
    ax.errorbar(x_count, fit_data, yerr=fit_std, ecolor='silver')
...
def PlotFitnessSampleES(walks, sample_folder, max_generations, exp_pop_size):
    #destination_file=sample_folder+"/BE_plot"+PLOT_EXTENSION
    destination_file=sample_folder+"/FitnessES"
    ...
    for walk_id in walks:
        walk_folder=BuildWalkFolderStr(sample_folder,walk_id)
        all_files.append(walk_folder+"/vre.csv")

    #li = []

    all_fit_data=defaultdict(list)
    #fit_std=dict()
    ...
    for filename in all_files:
        ...
        for it_gen in range(max_generations):
            ...
            for it_ind in range(exp_pop_size):
                ...
                ind_fit=pop_data.iloc[ind_loc]["Fitness"]
                #gen_fit.append(ind_fit)
                all_fit_data[it_gen].append(ind_fit)
            #print(gen_fit)
            #print(np.mean(gen_fit))
            #fit_data[it_gen].append(np.mean(gen_fit))
            #fit_std[it_gen].append(np.std(gen_fit))
    ...
    for it_gen in range(max_generations):
        fit_data.append(np.mean(all_fit_data[it_gen]))
        fit_std.append(np.std(all_fit_data[it_gen]))
...
def main(args):
    ...
    exp_pop_size=experiment_details["population_size"].iloc[0]
    ...
    for sample_id in df_samples["ID"]:
        ...
        for walk_id in df_walks["ID"]:
            ...
            #walk_fitness[walk_id]=PlotFitnessES(walk_folder, exp_pop_size, max_generations)
            PlotFitnessES(walk_folder, exp_pop_size, max_generations)
        ...
        PlotFitnessSampleES(df_walks["ID"], sample_folder, max_generations, exp_pop_size)
